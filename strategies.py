...
if typing.TYPE_CHECKING:
    ...
    from connectors.BitmexConnection import BitmexConnection
...
class Strategy():
    def __init__(self, client: typing.Union['BitmexConnection', 'BinanceConnection'], contract: Contract, exchange:str, timeframe:str, balance_pct: float, take_profit:float, stop_loss:float, strat_name):
        self.client = client
    ...
    def parse_trades(self, price: float, size: float, timestamp: int) -> str:
        ...
        if timestamp_diff > 2000:
            logger.warning('%s %s: %s milliseconds of difference between current time and the trade time', self.exchange, self.contract.symbol, timestamp_diff)
        ...
        #Missing Candles
            ...
            for missing in range(missing_candles):
                ...
                new_candle = Candle(candle_info, self.tf, "parseTrade")
    ...
    def _open_position(self, signal_result:int):
        ...
        if order_status is not None:
            self._add_log(f'{order_side.capitalize()} order places on {self.exchange} | Status: {order_status.status}')
            ...
            new_trade = Trade({'time_of_execution': dt, 'time': int(time.time()*1000), 'entry_price': avg_fill_price, 'contract': self.contract,
...
class TechnicalStrategy(Strategy):
    def __init__(self, client, contract: Contract, exchange: str, timeframe: str, balance_pct: float, take_profit: float,stop_loss: float, other_params:typing.Dict):
